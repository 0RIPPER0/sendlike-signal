...
function startGroupTimer(code) {
  ...
  if (g.ttlMs > 0) {
    g.timer = setTimeout(
      () => disbandGroup(code, "Time limit reached"),
      g.ttlMs
    );
  }
}
...
io.on("connection", (socket) => {
  socket.on("createGroup", ({ name, ttlMinutes = 10 }, cb = () => {}) => {
    const code = genCode();
    const ttlMs =
      ttlMinutes === 0 ? 0 : Math.max(0, (ttlMinutes || 10) * 60 * 1000);
    ...
    cb({
      code,
      expiresAt: ttlMs ? createdAt + ttlMs : 0,
      hostId: socket.id
    });
  ...
  socket.on("joinGroup", ({ name, code }, cb = () => {}) => {
    ...
    cb({
      ok: true,
      expiresAt: g.ttlMs ? g.createdAt + g.ttlMs : 0,
      hostId: g.hostId
    });
  });
